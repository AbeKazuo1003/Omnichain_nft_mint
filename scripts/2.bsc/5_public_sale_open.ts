...
async function main() {
  ...
  const publicSalePrice = utils.parseEther(
    BSCCollectionConfig.publicSale.price.toString()
  );
  if (!(await contract.cost()).eq(publicSalePrice)) {
    console.log(
      `Updating the token price to ${BSCCollectionConfig.publicSale.price} ${
        network === 'bsc'
  ...
  if (
    ...
    console.log(
      `Updating the max mint amount per TX to ${BSCCollectionConfig.publicSale.maxMintAmountPerTx}...`
    );
