...
async function main() {
  ...
  // Check configuration
  if (PolygonCollectionConfig.whitelistAddresses.length < 1) {
    throw (
      '\x1b[31merror\x1b[0m ' +
  ...
  const leafNodes = PolygonCollectionConfig.whitelistAddresses.map((addr) =>
    keccak256(addr)
  );
  ...
  const whitelistPrice = utils.parseEther(
    PolygonCollectionConfig.whitelistSale.price.toString()
  );
  if (!(await contract.cost()).eq(whitelistPrice)) {
    console.log(
      `Updating the token price to ${PolygonCollectionConfig.whitelistSale.price} ${
        network === 'polygon'
  ...
  if (
    ...
    console.log(
      `Updating the max mint amount per TX to ${PolygonCollectionConfig.whitelistSale.maxMintAmountPerTx}...`
    );
